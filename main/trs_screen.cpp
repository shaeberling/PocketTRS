...
{
  top = nullptr;
  vi = TrsVirtualInterface::instance();
}
...
void TRSScreen::onBufferUpdated() {
  vi->setScreenBuffer(top->getBuffer());
  vi->setScreenSizeProvider([this]() {
    return std::make_tuple(this->getWidth(), this->getHeight());
  });
}
...
void TRSScreen::update(uint8_t* from, uint8_t* to) {
  assert(top != nullptr);
  top->update(from, to);
}
...
{
  if (top == nullptr || !isTextMode() || vi->printerRead() == 0xff) {
    return;
  }
  ushort pos = 0;
